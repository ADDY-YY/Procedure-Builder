...
export function ProcedureEditor({sections,onChange,depth=0,onUndo,onRedo,canUndo=false,canRedo=false}:{sections:ProcedureSection[];onChange:(p:ProcedureSection[])=>void;depth?:number;onUndo?:()=>void;onRedo?:()=>void;canUndo?:boolean;canRedo?:boolean}) {
  ...
  function newHowToSection(callout?:'important'|'warning'){const section=newSection();section.template='how-to';section.steps=[];section.callout=callout;return section;}
  return <>
    {!sections.length&&<p className="empty-inline">{isHowTo?'Start with the first action someone needs to take.':'Start with a section that groups related steps, such as Prepare or Complete the request.'}</p>}
    ...
    {sections.map((section,index)=>{
      ...
      const stepNumber=sections.slice(0,index+1).filter(item=>!item.callout).length;
      const howToCallout=isHowTo&&section.callout;
      return <SectionCard key={section.id} title={`${String(index+1).padStart(2,'0')}  ${howToCallout?section.callout==='warning'?'Warning':'Important information':section.title||`Untitled ${isHowTo?'step':'procedure section'}`}`} subtitle={isHowTo?'':`${section.steps.length} steps · ${section.decisions.length} decisions`} open>
        <div className="list-heading">{howToCallout?section.callout==='warning'?'WARNING':'IMPORTANT':isHowTo?'STEP':'SECTION'} {howToCallout?'':stepNumber}<ItemActions index={index} length={sections.length} onMove={to=>onChange(move(sections,index,to))} onDelete={()=>onChange(sections.filter((_,itemIndex)=>index!==itemIndex))} onDuplicate={()=>onChange([...sections,{...structuredClone(section),id:uid(),title:section.callout?'':section.title+' (copy)'}])}/></div>
        {howToCallout?<RichTextField label={section.callout==='warning'?'Warning':'Important information'} required value={section.purpose} onChange={purpose=>update(index,{...section,purpose})} placeholder="Write the callout…"/>:<><TextField label={isHowTo?'Action':'Section title'} value={section.title} required onChange={title=>update(index,{...section,title})}/>
        ...
        {!section.steps.length&&<p className="empty-inline">Start with the first action. Add important, warning, or decision cards only where they help the reader.</p>}
    ...
    {isHowTo?<div className="step-add-actions"><button className="add-wide" onClick={()=>onChange([...sections,newHowToSection()])}><Plus size={16}/>Add step</button><button className="secondary" onClick={()=>onChange([...sections,newHowToSection('important')])}><Plus size={15}/>Add important card</button><button className="secondary" onClick={()=>onChange([...sections,newHowToSection('warning')])}><Plus size={15}/>Add warning card</button></div>:<button className="add-wide" onClick={()=>{const section=newSection();onChange([...sections,section]);}}><Plus size={16}/>{depth?'Add subsection':'Add procedure section'}</button>}
  </>;
}
