...
export function ProcedureStepEditor({step,onChange,allowImage=true}:{step:Step;onChange:(s:Step)=>void;allowImage?:boolean}) {
  ...
  return <>
    <RichTextField label="Instruction" required value={step.instruction} onChange={instruction=>onChange({...step,instruction})} placeholder="Describe what the reader needs to do…"/>
    <details className="minor-details" open={substeps.length>0}>
  ...
  return <>
    ...
    {sections.map((section,index)=>{
      ...
      return <SectionCard key={section.id} title={`${String(index+1).padStart(2,'0')}  ${section.title||`Untitled ${isHowTo?'step':'procedure section'}`}`} subtitle={isHowTo?'':`${section.steps.length} steps · ${section.decisions.length} decisions`} open>
        ...
        {section.steps.map((step,stepIndex)=><div className="step-card" key={step.id}>
          <div className="step-heading"><span className="step-number">{stepIndex+1}</span><strong>Step {stepIndex+1}</strong><ItemActions index={stepIndex} length={section.steps.length} onMove={to=>update(index,{...section,steps:move(section.steps,stepIndex,to)})} onDelete={()=>update(index,{...section,steps:section.steps.filter((_,itemIndex)=>itemIndex!==stepIndex)})} onDuplicate={()=>update(index,{...section,steps:[...section.steps.slice(0,stepIndex+1),{...structuredClone(step),id:uid()},...section.steps.slice(stepIndex+1)]})}/></div>
          <ProcedureStepEditor step={step} allowImage={!isHowTo} onChange={nextStep=>update(index,{...section,steps:section.steps.map((item,itemIndex)=>itemIndex===stepIndex?nextStep:item)})}/>
        </div>)}
        {!section.steps.length&&<p className="empty-inline">Add the first step in this section.</p>}
        <button className="add-wide" onClick={()=>update(index,{...section,steps:[...section.steps,newStep()]})}><Plus size={15}/>Add step</button></>}
