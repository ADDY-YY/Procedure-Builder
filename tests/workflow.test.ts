...
for(const type of ['procedure','how-to','faq','reference','decision-table','job-aid-one-column','job-aid-two-column','quick-reference'] as const){const x=newDocument(type);x.title='Shared header';x.audience='Frontlines';x.owner='Operations';x.effectiveDate='2026-09-11';x.purpose='Test';x.fields.extension=['1234'];x.fields.ticket_link=['https://example.com/support'];x.fields.related_procedures=['[Account access procedure](https://example.com/access)'];if(type==='procedure'||type==='how-to'){x.procedures[0].title='Action';x.procedures[0].purpose='Complete the action.';x.procedures[0].steps=[];}else x.entries=[{id:'e',title:'Question',content:'Answer'}];const output=renderHtml(x);assert(output.includes('Shared header'));assert(output.includes('September 11, 2026'));assert(output.includes('Need Help'));assert(output.includes('Ext. 1234'));assert(output.includes('font-family:Arial'));}
...
const headedDecision=newDocument('decision-table');headedDecision.title='Routing';headedDecision.purpose='Route requests.';headedDecision.fields.column_1_title=['If this situation involves'];headedDecision.fields.column_2_title=['Contact this department'];headedDecision.entries=[{id:'route',title:'A card issue',content:'Card Services'}];const headedDecisionHtml=renderHtml(headedDecision);assert(headedDecisionHtml.includes('If this situation involves'));assert(headedDecisionHtml.includes('Contact this department'));
