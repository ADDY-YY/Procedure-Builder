...
const howTo=newDocument('how-to');howTo.title='Reset a profile';howTo.purpose='Restore access.';howTo.procedures[0].title='Open settings';howTo.procedures[0].purpose='Open profile settings.';howTo.procedures[0].steps=[];assert.equal(validate(howTo).length,0);const howToHtml=renderHtml(howTo);assert(howToHtml.includes('How To: Reset a profile'));assert(howToHtml.includes('Open profile settings.'));howTo.fields={required_access:['Sign in to the support console.'],required_apps_setup:['Open the member profile tool.'],pre_steps:['Close the previous profile.'],if_issue:['access remains blocked'],do_this:['Contact Identity Support.'],important_notes:['Do not reset a shared profile.'],submit_a_ticket:['Open an Identity Support ticket.'],contact_team:['Identity Support'],link_to_procedure:['Profile access procedure']};const completeHowTo=renderHtml(howTo);['Sign in to the support console.','Open the member profile tool.','Close the previous profile.','access remains blocked','Contact Identity Support.','Do not reset a shared profile.','Open an Identity Support ticket.','Identity Support','Profile access procedure'].forEach(value=>assert(completeHowTo.includes(value)));
