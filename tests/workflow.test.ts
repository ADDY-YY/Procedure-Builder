...
const dated=newDocument();dated.title='Dated';dated.purpose='Test';dated.procedures[0].title='Action';dated.procedures[0].steps[0].instruction='Complete the action.';dated.effectiveDate='2026-01-01';dated.owner='2026-09-11';assert(renderHtml(dated).includes('September 11, 2026'));assert(!renderHtml(dated).includes('January 1, 2026'));
...
for(const type of ['procedure','how-to','faq','reference','decision-table','job-aid-one-column','job-aid-two-column','quick-reference'] as const){const x=newDocument(type);x.title='Shared header';x.audience='Frontlines';x.owner='2026-09-11';x.purpose='Test';if(type==='procedure'||type==='how-to'){x.procedures[0].title='Action';x.procedures[0].purpose='Complete the action.';x.procedures[0].steps=[];}else x.entries=[{id:'e',title:'Question',content:'Answer'}];const output=renderHtml(x);assert(output.includes('Shared header'));assert(output.includes('September 11, 2026'));}
